...
# When folder with data is asked, must respond with /home/userpath/pathofthefolder/folder
# When folder to generate plots asked must only respond with the actual name, eventually, the program will create the folder on the DEKSTOP (Path DESKTOP it's hardcoded so must see)
# 
# You can only upload the data of 1 telescope
# 
#
#
# FEEL FREE TO CHANGE ANYTHING, THIS IS MY FIRST SCRIPT, SO IDEAS ACCEPTED
# Script made by Albert Saez Nunez, as43z on Github, 19-07-2019
...
def ReadFile(file_to_read):
    ...
    for line in fileopt:
        ...
        if not traspaser[0].find("#")  == 0:
            ...
            if MAG > 0: #Gets rid of Gap objects
...
def MakeHistogramRA(Objects, folder):
    ...
    #To get the User Path:
    pathusr = os.environ['HOME']

    #BIT ROUGH MAYBE CAN AUTOMATITZE:
    plt.savefig(pathusr + '/Desktop/' + folder  + '/Histog_RA.png')
    plt.clf()
    plt.cla()
    plt.close()
...
def MakeHistogramDEC(Objects, folder):
    ...
    #BIT ROUGH MAYBE CAN AUTOMATITZE:
    plt.savefig(pathusr + '/Desktop/' + folder  + '/Histog_DEC.png')
    plt.clf()
    plt.cla()
    plt.close()
...
os.system("mkdir " + os.environ['HOME'] + "/Desktop/" + fld)
